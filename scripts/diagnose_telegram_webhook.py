...
def main():
    try:
        ...
        script_text = (ROOT / "scripts" / "telegram_webhook.py").read_text(encoding="utf-8", errors="replace")
        ...
        require('os.environ.get("TELEGRAM_WEBHOOK_SECRET") or ""' in routes_text, "webhook must compare against raw TELEGRAM_WEBHOOK_SECRET")
        require('request.headers.get("X-Telegram-Bot-Api-Secret-Token") or ""' in routes_text, "webhook must compare exact Telegram secret header")
        ...
        require("last_accepted_update_at" in routes_text, "last accepted update health marker missing")
        ...
        require("def reset_webhook" in script_text and "deleteWebhook" in script_text and "setWebhook" in script_text, "reset webhook command missing")
        require("TELEGRAM_WEBHOOK_TEST_CHAT_ID" in script_text, "start webhook test support missing")
