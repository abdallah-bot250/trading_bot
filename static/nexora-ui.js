(() => {
  ...
  const marketFallback={BTCUSDT:{label:"BTC/USDT",icon:"BTC",cls:"btc"},ETHUSDT:{label:"ETH/USDT",icon:"ETH",cls:"eth"},SOLUSDT:{label:"SOL/USDT",icon:"SOL",cls:"sol"},BNBUSDT:{label:"BNB/USDT",icon:"BNB",cls:"bnb"},XRPUSDT:{label:"XRP/USDT",icon:"XRP",cls:"xrp"},ADAUSDT:{label:"ADA/USDT",icon:"ADA",cls:"ada"},DOGEUSDT:{label:"DOGE/USDT",icon:"DOGE",cls:"doge"}};
  ...
  function renderMarketBar(){
    ...
    bar.innerHTML=symbols.map((symbol)=>{const item=marketFallback[symbol];return `<article class="nx-market-card" data-nx-symbol="${symbol}"><div class="nx-market-left"><span class="nx-coin ${item.cls}">${item.icon}</span><span class="nx-symbol">${item.label}</span></div><strong class="nx-price">--</strong><span class="nx-change is-muted">Live unavailable</span></article>`}).join("")+`<div class="nx-market-meta"><span>LIVE</span><span class="nx-dot"></span><small>Market data</small></div>`;
    document.body.insertBefore(bar, document.body.firstChild.nextSibling);
  }
  function applyMarketValues(values){symbols.forEach((symbol)=>{const node=document.querySelector(`[data-nx-symbol="${symbol}"]`);if(!node)return;const item=values[symbol]||cache.get(symbol);const price=node.querySelector(".nx-price");const change=node.querySelector(".nx-change");if(price)price.textContent=item?formatPrice(item.price||item.lastPrice):"--";if(change){const ch=Number(item?.change??item?.priceChangePercent);const txt=Number.isFinite(ch)?`${ch>=0?"+":""}${ch.toFixed(2)}%`:"Live unavailable";change.textContent=txt;change.classList.toggle("down",txt.startsWith("-"));change.classList.toggle("is-muted",!Number.isFinite(ch));}})}
  async function refreshMarket(){if(!(isDashboard||isAdmin))return;try{const url=`https://api.binance.us/api/v3/ticker/24hr?symbols=${encodeURIComponent(JSON.stringify(symbols))}`;const response=await fetch(url,{cache:"no-store"});if(!response.ok)throw new Error("market api failed");const data=await response.json();const values={};data.forEach((row)=>{if(!row||!row.symbol||!symbols.includes(row.symbol))return;values[row.symbol]={price:row.lastPrice,change:row.priceChangePercent};cache.set(row.symbol,values[row.symbol]);});applyMarketValues(values)}catch(_){applyMarketValues({})}}
